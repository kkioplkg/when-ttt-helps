...
IS2 = os.path.join(REPO, "paper", "is2")               # the manuscript tree
...
def collect():
    ...
    for d in PAPER_DIRS:
        ...
        #
        # ".jpg" is in this whitelist for exactly one pair of files: the two
        # author photographs `paper/figures/bio_yi.jpg` and
        # `paper/figures/bio_wang.jpg`, which the article's "Author
        # biographies" section wraps through cas-common's `\bio' mechanism.
        # They are `\includegraphics` targets of the shipped main.tex, so an
        # archive that carries the source without them carries a source that
        # does not compile -- the same defect class the review package's
        # standalone build gate exists to catch.  No other .jpg is reachable
        # from PAPER_DIRS; if one ever is, it ships, which is why the payload
        # census in the manifest is generated and not typed.
...
def selector_release_facts():
    ...
    """
    fresh = os.path.join(REPO, "results", "is_fresh")
...
ABS_PATH_EXCEPTIONS = {
    ...
    # TWELVE ENTRIES USED TO SIT HERE, AND ARE GONE BECAUSE THE FILES ARE NOW
    # CLEAN, NOT BECAUSE THE RULE WAS RELAXED.  They were the remote-host GPU
    # runners under experiments/ttt/{e2_cifar,e4_gpt2}, the e4 job list, the
    # e4 smoke transcript and the three vec_rerun drivers, and the reason
    # recorded against them was that they shipped UNMODIFIED as the record of
    # what was executed.  That reason had quietly stopped being the whole
    # truth: the result records those defaults were said to "reproduce" have
    # themselves carried the placeholder <RUN_ROOT> since the record-corpus
    # sanitization (BUILD_ENVIRONMENT.md 6.4), so the shipped scripts were the
    # only place the run host's literal prefix survived, and the archive was
    # asserting a correspondence with the records that no longer held byte for
    # byte.  The prefix is now substituted in the SOURCE TREE -- the run host's
    # scratch root by a relative `workdir/`, its interpreter by a bare
    # `python`/`pip`, and the vec_rerun drivers' project root by a REQUIRED
    # ${TTT_ROOT} that aborts with a message when unset -- so every suffix,
    # flag and ordering that made these files a record is preserved and only
    # the machine-identifying prefix is gone.  6.4 records the substitution
    # alongside the record-corpus one, which is what keeps them readable as a
    # record.  The gate asserts every declared exception is STILL DIRTY, so
    # these entries could not have been left behind: leaving them would have
    # turned the gate red, which is the gate working.
    #
    # The ImageNet-C runner's nine entries left this map earlier, for the
    # other reason a stale exemption dies: the runner is gone from the payload
    # -- see DROPPED_PAYLOAD.  The gate asserts that every file named here
    # still ships as well, so neither kind of staleness can survive here.
    #
