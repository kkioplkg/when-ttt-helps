...
|---|---|---|
| pages | **37** | **53** |
| `.pdf` size | 1,276,209 bytes | 946,595 bytes |
| `.pdf` SHA-256 | `82451eea52a8eb908f8c2263429bb1765c15b6298c7a646777af8ce706f94514` | `02373010b0baddd9bdd47a67573d94cc2faf6e962fd766b0489779de622baace` |
| `.tex` SHA-256 | `e86bcc5d81281802fe7136dd7b541fb44d293a58867490d51006fe507dc52288` | `a027b769fae7ee57e9c93dcbe33b95c86424d30c473e2e13735c211c6e959987` |
| `.bbl` SHA-256 | `957cdc19656447249cfe1e086ce779488be43574becee7ef039fb699ea96151c` | `e7d1f1f28f9177d96d46e0b7e36a8cbdb325e445f449dec268a811eb924dbecb` |
| LaTeX errors | 0 | 0 |
| undefined references / citations | 0 | 0 |
| multiply-defined labels | 0 | 0 |
| `.log` size / SHA-256 | 39,393 bytes / `c8bf7336acf368afc3b70d5cb795d749deba1a18681ebb9242e2c313ed7b1e3a` | 23,851 bytes / `a77323cb6cd28cb040ee2739f695c343bb63caded2faf96b77fd8bc7ae18cac8` |
| final-page footer | `Page 37 of 37` (verify this; see section 1) | none (the supplement carries no journal front matter) |
...
|---|---|---|---|
| article | `Overfull \hbox` | 117.08 pt | detected at line 175 |
| supplement | `Overfull \hbox` | 1.15 pt | in paragraph at lines 9--138 |

1 of these is the **front-matter** box `cas-sc` emits at `\maketitle` (main.tex line 175, read from the source) out of the placeholder author block. Its size is invariant to shortening any single placeholder string and it is produced by the class's front-matter assembly rather than by a paragraph of ours; it is the box that has to be re-judged on the build carrying the real metadata, which is a human item.

The remaining 1 box is **not** explained by the placeholder metadata and is listed above with the source lines to fix. An undispositioned sized box means a page may be clipped with nobody having looked.

**Placeholder status, read from the source rather than described.** `paper/main.tex` carries 19 placeholder fields on live lines (author names, ORCIDs, e-mail, CRediT and affiliations). The affiliation fields `addressline`, `city`, `country`, `organization`, `postcode` are **active** (set on live lines, not commented out), so there is nothing to *restore*: they must be **replaced** with the real values. Fields present only inside comments: `organization`.

The supplement's own front matter carries 5 placeholder fields on live lines.
...
> `ABS_PATH_EXCEPTIONS`: this document, the verbatim `pdflatex` transcript,
> the `pip freeze --all` provenance record, and the original remote-host GPU
> runners, smoke transcripts and job files that ship unmodified as the record
> of what was executed. The gate prints that list, with a per-file reason and
> count, on every run, and the census below is generated from the same run.
...
| `paper/is2/paper/BUILD_ENVIRONMENT.md` (4 paths) | this file, whose *purpose* is to record the reference build machine. §2 must name the TEXMF root for the build to be reproducible at all, and §6.4 — this section — discloses every other exception. |
| `experiments/ttt/e2_cifar/{adapt_cifar,delta_feat,train_source,train_recon_head}.py` (3+4+3+1) | the **original GPU runners, shipped as run**. Their `argparse` defaults are the recorded configuration that the result records' `meta/argv` fields reproduce. Rewriting a default would make the shipped script differ from the executed one — a worse defect than disclosing that the run host kept data under a scratch prefix. |
| `experiments/ttt/e4_gpt2/{run_e4,prepare_data,delta_proxy_v2}.py` (3+2+3) | same. |
| `experiments/ttt/e4_gpt2/jobs_e4.txt` (14) | the **literal command lines executed on the run host**. They are a transcript, not a template; editing them would falsify what ran. |
| `experiments/ttt/e4_gpt2/SMOKE.md` (10) | a remote-host smoke transcript, same reasoning. |
| `pip-freeze-full.txt` (268 paths) | the verbatim `pip freeze --all` transcript of the build interpreter. The conda-prefix and user-site locations in it *are* the provenance record — they are what distinguishes the mixed installation `RESOLVER_TRANSCRIPT.md` section 3 reasons about from a clean one — so rewriting them would destroy the evidence. Nothing installs from this file: the installable pins are `requirements-analysis.txt` and `requirements-experiment.txt`, and both are clean. |

The ImageNet-C runner and its job files were nine further entries in this
map. They are gone from it because the runner is gone from the payload: this
submission does not report that experiment, so neither its code nor its
records ship, and `INDEX.md` lists that omission with its reason. The gate
asserts that every file named here still ships, so a stale exemption cannot
survive the removal of the file it exempted.

None of these is executed by any analysis in the archive: the CPU
re-analysis path documented in `COMMANDS.md` touches none of them, so a
reader never meets a path that resolves nowhere. **Every retained path is in
an enumerated file; within the scope stated above the count elsewhere is
zero.** The number of exception files and the number of paths they retain are
in the generated census below, printed by the gate that measured them.
...
exceptions, over 384 JSON members (129,767 string leaves parsed) and 226 text
members; 25 binary members out of scope, 34 portable shebangs excluded; 18
declared exceptions holding 800 matching contexts, which contain 829 absolute-path occurrences.**
...
Its scope is **all 635 entries** of the archive: the 625 manifested payload
files plus the 10 generated root entries. 384 + 226 + 25 = 635, so every entry falls in
...
> 625 payload files — while the claim above, and the gate's own header
