...
# .../paper/is2/tools/build_env_section3.py -> parents[3] is the repository
...
def build_4() -> str:
    ...
    if fm:
        out += [
            "",
            f"{len(fm)} of these {'is' if len(fm) == 1 else 'are'} the "
            f"**front-matter** box `cas-sc` emits at `\\maketitle` "
            f"(main.tex line {mt}, read from the source) out of the "
            f"placeholder author block. Its size is invariant to shortening "
            f"any single placeholder string and it is produced by the class's "
            f"front-matter assembly rather than by a paragraph of ours; it is "
            f"the box that has to be re-judged on the build carrying the real "
            f"metadata, which is a human item.", ""]
    if other:
        out += [
            f"The remaining {len(other)} "
            f"{'box is' if len(other) == 1 else 'boxes are'} **not** "
            f"explained by the placeholder metadata and "
    ...
    a = status["article"]
    out += [
        "**Placeholder status, read from the source rather than described.** "
        ...
        + " are **active** (set on live lines, not commented out), so there "
          "is nothing to *restore*: they must be **replaced** with the real "
          "values. "
        ...
        f"{'' if status['supplement']['placeholders'] == 1 else 's'} on live "
        "lines.",
        "",
        END4]
    return "\n".join(out)
