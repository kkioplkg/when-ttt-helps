...
| Re-run the independent closure verifier's report | no — `experiments/results/is_fresh/closure/json/VERIFY_FINAL.json` ships here |
...
| unpacks to | `experiments/results/is_fresh/closure/records/` |

A member-by-member manifest **ships in this repository** at
`experiments/results/is_fresh/closure/CLOSURE_RECORDS_MANIFEST.json`, with a
...
| unpacks to | `experiments/results/is_fresh/e3_vectors/` |

The manifests and provenance for this set **do** ship here:
`experiments/results/is_fresh/e3_vectors/REPLICAS_MANIFEST.json`,
...
| | |
|---|---|
| size | 62,602,604 bytes (59.7 MiB) |
| sha256 | `1aa324f4711cd8fa4d91c90174268b7dbee04525d25207cb6af4028aa2cb821e` |
| contents | 635 entries, 234 MB uncompressed |
...
`paper/is2/tools/make_release_zip.py`, which is in this repository, so it can
be rebuilt.

## Raw record sets held back from this repository

| Path in the deposit | Files | Size | What it is |
|---|---:|---:|---|
| `experiments/results/e2/` | 24 | 140.1 MB | CIFAR-10/100-C per-instance adaptation traces (TENT, TTT-rot, TTT-mask, PL; main runs, batch sweeps, calibration) |
| `experiments/results/e4/` | 15 | 15.9 MB | GPT-2 per-document adaptation traces across `wikitext`/`pubmed`/`code`/`legal` |
| `experiments/results/e5/` | 12 | 10.2 MB | feature-shift (`delta_feat`) and LR-ablation per-instance dumps |
| `experiments/results/is_fresh/e3_vectors/` | 24 | 35.6 MB | the `.npz` + per-example `.json` payloads described above |
| `experiments/results/is_fresh/e2_gn/` | 3 | 17.3 MB | GroupNorm-lane per-instance JSONs (`cifar10_tent_main`, `f15_partial_cells`, `delta_feat_fresh`) |
| `experiments/results/is_fresh/closure/records/` | 71 | 128 MB | the closure records (`closure_records.zip`) |
...
- `experiments/results/is_fresh/e2_gn/cifar10_resnet26ttt_s20260806.pt` — 2,640,259 bytes
- `experiments/results/is_fresh/e2_gn/f15_train_partial.pt` — 2,638,813 bytes

Both are reproducible from `experiments/ttt/e2_cifar/train_source.py` with the
